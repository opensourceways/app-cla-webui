...
export default {
  ...
  methods: {
    ...
    getSignPage(resolve) {
      let applyTo = '';
      this.loginType === this.corporation
        ? (applyTo = this.loginType)
        : (applyTo = this.individual);
    ...
    setFields(key) {
      for (let i = 0; i < this.signPageData[key].fields.length; i++) {
        for (let j = i + 1; j < this.signPageData[key].fields.length; j++) {
          if (
            Number(this.signPageData[key].fields[i].id) >
            Number(this.signPageData[key].fields[j].id)
          ) {
