...
import { useLoading, LoadingPropsT } from '@opensig/opendesign';
interface RequestConfig<D = any> extends AxiosRequestConfig {
  data?: D;
  shownLoading?: {
    opt?: Partial<LoadingPropsT>;
    wrap?: Ref<HTMLElement> | HTMLElement | string;
  }; // 是否弹出错误提示框
...
const requestInterceptorId = request.interceptors.request.use(
  (config: RequestConfig) => {
    if (loadingCount === 0 && config?.shownLoading) {
      loadingInstance = useLoading(
        config.shownLoading.opt,
        config.shownLoading.wrap
      );
